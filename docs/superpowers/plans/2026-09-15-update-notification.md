...
test('aos.js dispatches the three update commands and rejects a bad flag', () => {
  const w = cmdWorld();
  const env = { ...process.env, AOS_CONFIG: path.join(w.configDir, 'agenticos.json'), HOME: w.configDir };
...
```js
test('collectConfig surfaces the update flag for the HUD', () => {
  const w = vaultWorld();
  const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aos-cfg-'));
  const { collectConfig } = require(path.join(__dirname, '..', 'brain', 'scripts', 'collectors', 'config.js'));
  assert.equal(collectConfig({ vault: w.vault, claudeConfigDir: configDir }).updates, undefined,
    'absent before the first check');
  U.writeState(w.vault, behindState());
  assert.deepEqual(collectConfig({ vault: w.vault, claudeConfigDir: configDir }).updates, {
...
Run: `node --test cli/update-check.test.js`
Expected: FAIL — `updates` is `undefined` after the store is written.
...
Run: `node --test cli/update-check.test.js && npm run build -w obsidian-plugin`
Expected: PASS, and the TypeScript build succeeds.

- [ ] **Step 5: Commit**

```bash
git add brain/scripts/collectors/config.js obsidian-plugin/src/data/snapshot.ts cli/update-check.test.js
