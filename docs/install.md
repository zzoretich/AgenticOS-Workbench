...
| `aos graph [status\|build\|on\|off]` | the vault knowledge graph (graphify, pinned): `status` prints the installed and pinned versions, the last build, counts, hubs and communities; `build` runs the structural pass now; `off` stops the `graph-build` scan stage (the graph and graphify stay), `on` resumes it |
...
- `skills/` — the 17 commands and 7 skills of `plugin/` as Codex skills, listed as `agenticos:<name>` (`$agenticos:wrap`, `$agenticos:remember`, …), with every Claude Code idiom rewritten; the launcher is `<plugin root>/bin/aos`, two folders above each skill's folder.
