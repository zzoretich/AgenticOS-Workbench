...
AgenticOS Workbench runs on macOS and Linux with Node 20+, Claude Code and/or Codex CLI (logged in), and optionally Obsidian and Ollama. **Windows is not supported in v1** (the launcher is POSIX `sh`, scheduling uses launchd/cron).
...
1. **Preflight** — Node ≥ 20; the hosts (see [Hosts](#hosts)): `claude` on PATH and logged in (`claude auth status`) and/or `codex` on PATH and logged in (`codex login status`), at least one; Obsidian detected (optional); python3 ≥ 3.9 (checked only with `--cost`); Ollama on `127.0.0.1:11434` (informational).
...
**What `aos init --host codex` writes.** Codex has a hook system shaped like Claude Code's, so the runtime is shared and only the wiring differs:
...
**Trust.** Codex quarantines hooks until you have reviewed them: open `codex`, run `/hooks`, and trust the AgenticOS entries once. They keep their trust across `aos upgrade` because the command text never changes; only moving the vault changes it.

**What differs from Claude Code.** Codex reports tool payloads as JSON strings (handled), fires SessionEnd when a thread is closed or archived rather than when the TUI exits (so the background wrap can run later; `$wrap` in-session is immediate), and keeps its own built-in *memories* feature, which `aos` neither reads nor disables (doctor shows an info row when it is on). The Chief of Staff duties, `prompt` routines and the HUD's Chat tab still run through the `claude` CLI; on a Codex-only machine they stay inactive.
...
`sh cli/rehearsal/first-run.sh` does the whole first run in a temporary HOME and config dir with a fake `claude`, then checks recall and `wrap_session` over MCP and uninstalls. `sh cli/rehearsal/codex-host.sh` does the same for a Codex-only machine with a fake `codex`: the written hook commands are run for real (the conventions injection, the Stop hook's `{}`, the first-prompt context), then doctor and uninstall. CI runs both on Ubuntu and macOS.
