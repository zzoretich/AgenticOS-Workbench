...
| `aos upgrade` | `claude plugin marketplace update` + `plugin update`, re-vendor the runtime and bundle, add new config keys (your values win), rebuild indexes. Never touches memory, notes, or persona. Runs the CLI that ships with the source it upgrades from (the launcher execs the vault's previous copy), so a step that is new in that release runs the first time. |
