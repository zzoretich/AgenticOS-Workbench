...
- [ ] On a machine wired directly before this release, `aos upgrade` installs the plugin, prints `direct wiring removed (5 hook entries, the MCP registration, 25 skills)` and the `/hooks` reminder; after trusting the entries once, a second `aos upgrade` (or a version bump) leaves them trusted.
