...
- [ ] A `codex exec "say ok"` session (hooks trusted) leaves `brain/_index/agent-runs/live/sess-<id>.ndjson` behind; after `telemetry.staleAfterMinutes` (set it to 1 for the check) the next session start or stop of either host runs `reconcile-sessions`: the live file is gone, `runs.jsonl` has a `host: "codex"` row with `end_reason: "reconciled"` and the model name, and the pipeline ledger shows `auto-cost` and `auto-wrap` runs for that session. `aos reconcile-sessions --dry-run --force` lists what a sweep would do.
