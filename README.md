...
| <img src="docs/assets/icon-hud.svg" width="48" alt="" /> | **The Agentic OS HUD.** An Obsidian plugin with eleven tabs — Pulse (pipeline LEDs, live agent runs, the fix queue), To-Do (your `TODO.md` in Obsidian Tasks syntax, grouped by due date and priority), Proposals (the Chief of Staff's pending proposals, backlog and decision history, with a count badge), Spaces, Memory (list or force graph), Runs, Routines, Skills (every Claude Code and Codex skill, runnable on either host), Agents (every Claude Code and Codex agent, startable on either host), an optional Chat and an optional embedded terminal — plus ⚙ Settings pinned to the rail's foot, and a sidebar carrying the update and heartbeat pills. |
| <img src="docs/assets/icon-commands.svg" width="48" alt="" /> | **One settings panel.** ⚙ Settings, at the bottom-left of the Workbench rail, shows every setting of the whole system in one place: master switches (background AI, the Chief of Staff, routines, the graph, cross-review, costing, telemetry, update checks, sharing), then each section — provider and models, spend limits with today's spend against each cap, memory and scanning, the Chief of Staff, routines, the graph, cross-review, telemetry, updates — and the plugin's own settings. Every change goes through `aos config`, so it lands in the file that actually wins, atomically; raising a cap or widening an agent's autonomy asks first, and steps a change leaves (a schedule sync) become one-click buttons. Rows for a host you have not enabled are dimmed with the command that turns it on. |
...
plugin             the Claude Code plugin: hooks.json, .mcp.json, bin/aos, 19 commands, 9 skills (also the source of codex-plugin)
codex-plugin       the Codex plugin, generated from plugin/ by npm run build:codex-plugin: hooks.json, .mcp.json, bin/aos, 26 skills
