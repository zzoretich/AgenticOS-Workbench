...
5. Run `{{NODE}} {{VAULT}}/brain/scripts/persona/reflect.js inputs --days 28` — the month's evidence in one JSON: the ledger summary, each duty's last run and fail streak, spend per duty, the feedback memories and drafts written, sessions per day, and the signals the nightly reflect has not drained yet (read them; do not drain — the nightly `reflect-daily` duty does).
