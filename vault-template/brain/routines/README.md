...
- `prompt` sends the body to headless Claude with the tools in `routines.tools`, capped by `routines.perDayUsd`.
- `command` runs `argv: [program, arg, …]` directly (no shell) with the vault as the working directory.
