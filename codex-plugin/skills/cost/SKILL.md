...
- If a session UUID was given: Bash `aos auto-cost --cost-one <uuid>` (fallback: `sh "<plugin root>/bin/aos" auto-cost --cost-one <uuid>`). The script finds the transcript under `<claudeConfigDir>/projects/*/<uuid>.jsonl`, runs the analyzer, and patches `cost_usd` in `brain/_index/agent-runs/runs.jsonl`. Do not target the live session; it is not finalized yet.
