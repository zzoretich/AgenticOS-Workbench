...
> Host: Codex CLI, through the `agenticos` plugin. Invoke as `$agenticos:routines`. `<plugin root>` is the plugin's folder, two levels above the folder that holds this SKILL.md; the `aos` launcher is `<plugin root>/bin/aos`. The tools of the `agenticos` MCP server are named `mcp__agenticos__<tool>`.

Run `aos routines the text the user wrote after `$agenticos:routines`` via Bash (fallback: `sh "<plugin root>/bin/aos" routines the text the user wrote after `$agenticos:routines``); with no arguments run `aos routines list`. The one verb that is not a passthrough is `cloud` (below).
