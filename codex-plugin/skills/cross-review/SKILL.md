...
> Host: Codex CLI, through the `agenticos` plugin. Invoke as `$agenticos:cross-review`. `<plugin root>` is the plugin's folder, two levels above the folder that holds this SKILL.md; the `aos` launcher is `<plugin root>/bin/aos`. The tools of the `agenticos` MCP server are named `mcp__agenticos__<tool>`.
...
`RUNNER` below means `sh "<plugin root>/bin/aos" cross-review`. Every command already carries `--host codex`,
...
Run `RUNNER preflight --host codex`. It prints the roles, each CLI's path, version and login (no model call), today's
...
When the agenticos memory tools are available, query `recall` (`mcp__agenticos__recall`) for the
project, its components and past decisions, and `feedback_rules` (`mcp__agenticos__feedback_rules`)
for the index of the user's standing instructions; read the ones that bear on the plan with `memory_read`
(`mcp__agenticos__memory_read`). Put relevant hits in the assumptions ledger with their vault paths; a
...
```sh
sh "<plugin root>/bin/aos" cross-review review --host codex --repo <repo> --plan <plan> [--model <m>] [--effort <e>]
```
...
- **REVISE:** arbitrate each finding. Make warranted plan changes, reject unsupported ones with reasons, and record
  ...
  ```sh
  sh "<plugin root>/bin/aos" cross-review review --host codex --repo <repo> --plan <plan> --prior <previous result.json> --feedback <dispositions file>
  ```
...
```sh
sh "<plugin root>/bin/aos" cross-review check --host codex --repo <repo> --plan <plan> --approval <approved result.json>
```
...
```sh
sh "<plugin root>/bin/aos" cross-review build --host codex --builder codex --repo <repo> --plan <plan> --approval <approved result.json> --proof "<proof command>"
```
...
```sh
sh "<plugin root>/bin/aos" cross-review inspect --host codex --builder <claude|codex> --repo <repo> --plan <plan> --base <pre-build commit>
```
...
independence, and the spend (the sum of each result's `usd`). Suggest `$agenticos:remember <decision> #promote` for a decision
