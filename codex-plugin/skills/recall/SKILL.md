---
name: recall
description: Ranked, recency-weighted recall over the whole vault — memories, patterns, daily notes, and the persona journal. Use when the user asks "when did I last touch X", "what did we decide about Y", "have we seen this before", "search my memory", "search the vault", "$agenticos:recall", or at the start of any non-trivial task to load prior context before working.
---
<!-- generated by tools/build-codex-plugin.js from plugin/; edit the source there, not this file -->

> Host: Codex CLI, through the `agenticos` plugin. Invoke as `$agenticos:recall`. `<plugin root>` is the plugin's folder, two levels above the folder that holds this SKILL.md; the `aos` launcher is `<plugin root>/bin/aos`. The tools of the `agenticos` MCP server are named `mcp__agenticos__<tool>`.

# Recall

Ranked retrieval over the vault's living record through the `recall` tool of the
`agenticos` MCP server (`mcp__agenticos__recall`): BM25 + recency boost + 1-hop
...
3. `memory_read` (`mcp__agenticos__memory_read`) at most the top 1-2 paths that look load-bearing. Never pull more than 2 full files into context from one recall pass.
