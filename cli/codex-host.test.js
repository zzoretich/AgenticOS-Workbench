...
test('generateSkills turns the real plugin into 24 marked skills, copies skill scripts, and never overwrites a foreign skill', () => {
  ...
  assert.equal(r.written.length, 21, 'seventeen commands and seven skills, two shared names (wrap, cost), one foreign (brain)');
  assert.equal(CH.countGenerated(dir), 21);
...
test('installCodexHost writes hooks.json, registers the MCP server once, generates skills; removeCodexHost reverses it', () => {
  ...
  assert.equal(r.skills.written.length, 22);
  ...
  assert.equal(rm.skills.length, 22);
...
test('removeDirectWiring takes out what a direct install wrote (hooks, MCP registration, skills) and nothing else (D5)', () => {
  ...
  CH.installCodexPlugin({ bin: '/x/codex', source: 'owner/repo', run });
  assert.deepEqual(CH.removeDirectWiring({ cfg: {}, bin: '/x/codex', run, env, dir }), { hooks: 5, mcp: true, skills: 22 });
...
test('codexPluginStatus reports the plugin, its MCP server and any direct wiring left behind', () => {
  ...
  assert.deepEqual(both.direct, { hookEvents: 5, mcp: true, skills: 22 });
