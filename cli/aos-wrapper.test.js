...
test('every contract name maps to a script path under brain/scripts', () => {
  ...
  assert.ok(src.includes('doctor|status|upgrade|uninstall|persona|cost|graph|routines|skills|agents|workspace|terminal|provider|update-check|update-status|update-notice) SCRIPT=cli/aos.js'), 'maintenance names reach cli/aos.js');
});
